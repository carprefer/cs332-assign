...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toList.groupBy(elem => elem).toList.map(elem => (elem._1, elem._2.length)).sorted
...
object Main extends App {
  val occur = Anagrams.sentenceOccurrences(List("Linux", "rulez"))
  val occur2 = Anagrams.sentenceOccurrences(List("Lin"))
  val combinationList = Anagrams.combinations(Anagrams.sentenceOccurrences(List("Linux", "rulez")))
  val dict = Anagrams.dictionaryByOccurrences.get(combinationList.head)
  println(combinationList)
  println(dict)

  println(Anagrams.subtract(occur, occur2))
  println(Anagrams.sentenceAnagrams(List("Linux", "rulez")))
}
