...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) Stream.empty
      ...
      val next = initial.flatMap{case (block, history) => newNeighborsOnly(neighborsWithHistory(block, history), explored)}
      initial ++ from(next, explored ++ initial.map{case (block, history) => block}.toSet)
    }
  }
